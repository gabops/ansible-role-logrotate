...
| logrotate_global_config | [] | Defines the configuration to be applied on `/etc/logrotate.conf` note that if this variable is empty as it is by default, the file `logtorate.conf` will not be touched at all. |
...
### Notes:
- All configurations defined in `logrotate_config_files` will be applied over the default directory `/etc/logrotate.d`. This behaviour can be changed if in the variable `logrotate_global_config` you declare the `include` directive. If you define:
...
The role will detect automatically the path after the `include` directive and will use that instead of the default one.
